<?php /* CLASSES $Id: libmail.class.php,v 1.1 2009-05-19 21:15:27 pkerestezachi Exp $ */
/**
 *	@package dotproject
 *	@subpackage utilites
*/
...
/**
 *	Define if email send in html format
...
//	$this->xheaders['To'] = implode( ", ", $this->sendto );
	...
	$this->xheaders["X-Mailer"] = "Proficient/Mailer";
...
function Send() {
	...
	$mail_to_folder = $AppUI->getConfig("mail_to_folder_enabled") ? $AppUI->getConfig("mail_to_folder_enabled") : false;
	$mail_to_folder_path = $AppUI->getConfig("mail_to_folder_path");
	$mail_disable_send = $AppUI->getConfig("mail_disable_send") ? $AppUI->getConfig("mail_disable_send") : false;
	$rta = true;
	if(!$mail_disable_send){
		$this->BuildMail();

		$strTo = implode( ", ", $this->sendto );

		// envoie du mailz
		$rta = @mail( $strTo, $this->xheaders['Subject'], $this->fullBody, $this->headers );

	}
	if($mail_to_folder){
		$mail_file = $mail_to_folder_path."/".date("YmdHis").".".uniqid("").".msg";
		$msg = $this->Get();
