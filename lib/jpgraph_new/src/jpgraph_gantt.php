...
// Ver:		$Id: jpgraph_gantt.php,v 1.1 2009-05-19 21:15:40 pkerestezachi Exp $
//
...
// PRIVATE Methods
	...
	$week = 1 + (date("z", $thursday) - (11 - date("w", mktime_fix(0, 0, 0, 1, 1, date("Y", $thursday)))) % 7) / 7;	  
	return $week;
    }
...
// PUBLIC METHODS	
	...
	$out_range = false;
	if( $en <  $aScale->iStartDate){
		$en =  $aScale->iStartDate;
		$out_range = true;
	}
		
	if ($st > $aScale->iEndDate){
		$st = $aScale->iEndDate+SECPERDAY;
		$out_range = true;
	}
...
// PUBLIC METHODS	
	...
	$out_range = false;
	if( $d <  $aScale->iStartDate){
		$d =  $aScale->iStartDate;
		$out_range = true;
	}
		
	if ($d > $aScale->iEndDate){
		$d = $aScale->iEndDate;
		$out_range = true;
	}	
