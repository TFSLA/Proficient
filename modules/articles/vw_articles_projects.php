...
//$xajax = new xajax();
//include ("./modules/articles/ajax.php");
...
if(count($sec_art)==0)
{
	$filtro_articlesection_con_permiso = " AND 1=99 ";//QUE NO TRAIGA nada si no tiene permisos
	$filtro_articlesection_con_permiso_file = " AND 1=99 ";//QUE NO TRAIGA nada si no tiene permisos
}
else
{
	$filtro_articlesection_con_permiso = " AND articlesection_id IN (". implode( ',', $sec_art).") ";
	$filtro_articlesection_con_permiso_file = " AND file_section IN (". implode( ',', $sec_art).") ";
}
...
if($sec_id!="")
{
$filtro_articlesection = " and articlesection_id='$sec_id'";
$filtro_articlesection_file = " and file_section ='$sec_id'";
}
...
$sql = "(
				...
				WHERE 1=1
					...
					$filtro_type
					$filtro_articlesection_con_permiso
...
$sql .= "UNION
				(SELECT
					DATE_FORMAT(file_date,'%Y%m%d') AS ord,
					file_section,
					file_owner,
					file_id,
					file_description,
				...
				WHERE
					...
					$filtro_type_file
					$filtro_articlesection_con_permiso_file
					$privateFilterFile
				)";
...
switch($accion){
	...
	case 'save_link':
       if ($_POST[article_id]=="0"){
		   ...
		   $query = "INSERT INTO articles (articlesection_id, date, articles_reads, user_id, title, abstract, body,  type,date_modified, is_protected, is_private) VALUES ('".$_POST[articlesection_id]."', '".$date."', '0', '".$AppUI->user_id."', '".$_POST[description]."', '".$_POST[href]."','".$_POST['abstract']."', '1','".$date_modified."', ".$_POST['is_protected'].", ".$_POST['is_private'].")";
		   echo $query; return;
	   }
       else{
		   ...
		   $query = "UPDATE articles SET articlesection_id= '".$_POST[articlesection_id]."',user_id='".$AppUI->user_id."',title='".$_POST[description]."',abstract='".$_POST[href]."',date_modified='".$date_modified."', is_protected=".$_POST['is_protected'].", is_private=".$_POST['is_private']." WHERE article_id='".$_POST[article_id]."' ";
	   }
...
?>
	<form name="frmEdit_link"  method="post">
		<input type="hidden" name="accion" value="edit_link">
	...
	<form name="frmEdit_file"  method="post">
		<input type="hidden" name="accion" value="edit_file">
		<input type="hidden" name="file_id" value="">
		<input type="hidden" name="origen" value="project">
	</form>
		...
		<tr class="tableHeaderGral">
			...
			<th align="left" width="1%">
			    ...
			    <select name="sec_id" class="text" onchange="submit()">
				    <option value=""><?php echo $AppUI->_('All sections');?></option>
					<?
					   $project_id = $_GET[project_id];
						...
						while ($rows = mysql_fetch_array($results, MYSQL_ASSOC)) {
						  ...
						  echo '<option ';
						  if($sec_id == $rows["articlesection_id"]) echo "selected";
						  echo ' value="'.$rows["articlesection_id"].'">'.$rows["name"].'</option>';
						}
					?>
				</select>
			</th>
		...
		<?php //if ($canEdit || $AppUI->user_id == $row["user_id"] ) {
		...
		?>
					     <?
							...
							if($row["type"]=="0")
							{
								...
								//echo "<a href=\"./index.php?m=articles&a=addeditarticle&from_kb=1&project_id=$project_id&article_id=".$row["article_id"]."\" title=\"".$AppUI->_('edit')."\">";
							    ?>
								...
								<?php } ?>
								<a href="javascript:edit_art('<? echo $row["article_id"]; ?>')" title="<?php echo $AppUI->_('edit');?>"><?php echo dPshowImage( './images/icons/edit_small.gif', 20, 20, '' ); ?></a>
								<?
							}
							elseif($row["type"]=="1")
							{
								if(!getDenyEdit("timexp")) {
								//echo "<a href=\"./index.php?m=articles&a=addeditlink&from_kb=1&project_id=$project_id&id=".$row["article_id"]."\" title=\"".$AppUI->_('edit')."\" >";
								?>
									<a href='javascript:report_hours(<? echo $row['article_id']; ?>,<? echo $row['type']; ?>);' >
									<img src='/images/icons/calendar_report.png' alt='Cargar Horas' border=0 style='height:18px;'></a>
								<?php } ?>
								<a href="javascript:edit_link('<? echo $row["article_id"]; ?>')" title="<?php echo $AppUI->_('edit');?>"><?php echo dPshowImage( './images/icons/edit_small.gif', 20, 20, '' ); ?></a>
								<?
							}
							else
							{
								if(!getDenyEdit("timexp")) {
								//echo "\n".'<a href="./index.php?m=files&a=addedit&file_id=' . $row["article_id"] . '" title="'.$AppUI->_('edit file').'">';
									?>
									<a href='javascript:report_hours(<? echo $row['article_id']; ?>,-1);' >
									<img src='/images/icons/calendar_report.png' alt='Cargar Horas' border=0 style='height:18px;'></a>
								<?php } ?>
								<a href="javascript:edit_file('<? echo $row["article_id"]; ?>')" title="<?php echo $AppUI->_('edit file');?>"><?php echo dPshowImage( './images/icons/edit_small.gif', 20, 20, '' ); ?></a>
								<?
							}
						?>
						 <?
		...
		<?php if($row["type"]!="0" && $row["type"]!="1")
			...
			<td align="left">
				...
				if($row["type"]!="0" && $row["type"]!="1")
				{
					...
					echo "<a href=\"javascript:popUp('index_inc.php?inc=./modules/files/show_versions.php&m=files&file_id={$row['article_id']}')\"
					title=\"{$row['abstract']}\">".
	...
	echo "<script language='Javascript'>
		...
		<tr bgcolor="#E9E9E9">
			...
			<td width="35">
			    ...
			    <a href="rss.php?s=<?=$sec_id ?>" target=_blank >
					<img src='./images/icons/rss_enabled.bmp' width='15'  border='0' alt='RSS'>
				</a>
				<? } else {?>
				    <img src='./images/icons/rss_disabled.bmp' width='14'  border='0' alt='RSS'>
				<? } ?>
			</td>
		</tr>
		<tr>
...
//Verifica permisos por documento
