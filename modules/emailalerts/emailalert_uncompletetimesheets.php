...
<?
	$today = new CDate();
  $resultut = mysql_query("SELECT * FROM users ORDER BY user_last_name, user_first_name ;");
  while ($rowut = mysql_fetch_array($resultut, MYSQL_ASSOC)) {
    $resultut2 = mysql_query("SELECT user_id, tasks.task_id, DATE_FORMAT(task_start_date,'%m/%d/%Y') as date FROM user_tasks, tasks WHERE user_tasks.user_id = {$rowut["user_id"]} AND tasks.task_id = user_tasks.task_id AND tasks.task_status=0 AND tasks.task_manual_percent_complete <> 100 AND tasks.task_start_date < CURDATE() ;");
    $tottasks=0;
    $maxdelay=0;
...
<tr>
	<td>
		<a href="mailto:<?php echo $rowut["user_email"];?>">
		<?php echo $rowut["user_username"];?></a>
	</td>
	<td>
