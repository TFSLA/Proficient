...
if ($selected && count( $selected )) {
	foreach ($selected as $key => $val) {
		if ( $task_priority == 'c' ) {
			...
			$sql = "UPDATE tasks SET task_manual_percent_complete=100 WHERE task_id=$val";
...
$sql = "
		 ...
		 users.user_username
		 FROM users, projects, tasks AS a, user_tasks
...
"		 AND (a.task_manual_percent_complete < 100 OR a.task_manual_percent_complete IS NULL)
		 ...
		 AND a.task_owner = users.user_id
...
<tr class="tableHeaderGral">
	...
	<th nowrap><?php echo $AppUI->_('Task Creator');?></th>
