...
include_once("./modules/webmail/include/header.inc.php");
//include_once("./modules/webmail/include/optionfolders.inc.php");

include_once("./modules/webmail/include/encryption.inc");
include_once("./modules/webmail/include/version.inc");
include_once("./modules/webmail/include/langs.inc");
include_once("./modules/webmail/conf/conf.inc");
include_once("./modules/webmail/conf/login.inc");

//set content type header
if (!empty($int_lang)){
	include_once("./modules/webmail/lang/".$int_lang."init.inc");
}else{
	include_once("./modules/webmail/lang/".$default_lang."init.inc");
}
...
if (!isset($session) || (empty($session))){	
    ...
    include_once("./modules/webmail/conf/defaults.inc");
	...
	if ((isset($user))&&(isset($password))&&(isset($host))){
		include("./modules/webmail/include/icl.inc");
		$user_name = $user;
		...
		if ($iil_conn){
            ...
            include("./modules/webmail/conf/custom_auth.inc");
            
			//if successful, start session
            if (empty($error)){
				if ((!isset($port))||(empty($port))) $port = 143;
                include("./modules/webmail/include/write_sinc.inc");
                if ($new_user){
                    include("./modules/webmail/conf/new_user.inc");
					$new_user = 1;
                }else{
					$new_user = 0;
				}
		...
		//make log entry
		$log_action = "log in";
		include("./modules/webmail/include/log.inc");
	}
}
...
if ((isset($session)) && ($session != "")){
	...
	*/
	...
	include("./modules/webmail/include/session_auth.inc");
	include("./modules/webmail/conf/defaults.inc");
	
	//authenticate
	if (!$authenticated){
		include_once("./modules/webmail/include/icl.inc");
	...
	if ($do_prefs){
		...
		include_once("./modules/webmail/lang/".$lang."init.inc");
		if (!empty($charset)){ 
		...
		if ((isset($update))||(isset($revert))){
			...
			include("./modules/webmail/include/save_prefs.inc");
...
include_once("./modules/webmail/include/stopwatch.inc");
$clock = new stopwatch;

$clock->register("start");
$tmpuser=$user;
include_once("./modules/webmail/include/super2global.inc");
$user=$tmpuser;

$clock->register("pre-header");
include_once("./modules/webmail/include/header_main.inc");
$clock->register("post-header");
include_once("./modules/webmail/include/ryosdates.inc");
include_once("./modules/webmail/include/icl.inc");
include_once("./modules/webmail/include/main.inc");
include_once("./modules/webmail/include/cache.inc");
$clock->register("includes done");
	if (!isset($folder)){
		echo "Error: folder not specified";
		exit;
	}
	include_once("./modules/webmail/lang/".$my_prefs["lang"]."defaultFolders.inc");
	include_once("./modules/webmail/lang/".$my_prefs["lang"]."main.inc");
	include_once("./modules/webmail/lang/".$my_prefs["lang"]."dates.inc");
	...
	if ((!empty($search)) || (!empty($search_criteria))){
		include("./modules/webmail/lang/".$my_prefs["lang"]."search_errors.inc");
		$criteria="";
		$error="";
		$date = $month."/".$day."/".$year;
	...
	*/
	...
	/*  start form */
	echo "\n<form name=\"messages\" method=\"POST\" action=\"?m=webmail\">\n";			
	...
	echo "<td align=left valign=bottom>\n";
		...
		if (strcasecmp("INBOX", $folder)==0)
			echo "[<a href=\"?m=webmail&session=$user&folder=$folder\" class=\"mainHeadingSmall\">".$mainStrings[17]."</a>]";
        if (strcmp($folder,$my_prefs["trash_name"])!=0)
			echo "[<a href=\"?m=webmail&session=$user&folder=$folder&delete_all=1\" class=\"mainHeadingSmall\">".$mainStrings[18]."</a>]";
		echo "</span>\n";
	echo "</td>\n";
	echo "<td align=\"right\" valign=\"bottom\" class=\"mainHeadingSmall\">";
		...
		if ($my_prefs["show_quota"]=="m"){
			...
			include("./modules/webmail/lang/".$my_prefs["lang"]."quota.inc");
	...
	if ($delete_all==1){
		...
		echo "<span class=\"small\">[<a href=\"?m=webmail&session=$user&folder=$folder&delete_all=2&delete_all_num=$total_num&submit=Delete\">";
			echo $mainStrings[18]."</a>]</span>";
		echo "<span class=\"small\">[<a href=\"?m=webmail&session=$user&folder=$folder\">".$mainStrings[19]."</a>]</span>";
	}
		...
		if (sizeof($headers)>0){			
			...
			if ($num_items > $num_show){
				if ($prev_start < $start){
					...
					echo "[<a href=\"?m=webmail&user=$sid&folder=".urlencode($folder).$args."\" class=\"mainLightSmall\">";
			...
			***/
			if (strpos($my_prefs["main_toolbar"], "t")!==false){
				include("./modules/webmail/include/main_tools.inc");
			}
			...
			echo "<tr bgcolor=\"".$my_colors["main_head_bg"]."\">\n";
				...
				$check_link.="\n--></SCRIPT><NOSCRIPT>";
				$check_link.="<a href=\"?m=webmail&folder=".urlencode($folder)."&start=$start&session=$user&sort_field=$sort_field&sort_order=$sort_order&check_all=1\"><b>+</b></a>|";
 				$check_link.="<a href=\"?m=webmail&folder=".urlencode($folder)."&start=$start&session=$user&sort_field=$sort_field&sort_order=$sort_order&uncheck_all=1\"><b>-</b></a>";
				$check_link.="</NOSCRIPT>";
				$tbl_header["c"] = "\n<td>$check_link</td>";
			...
			if ($MOVE_FIELDS){
				echo "<tr bgcolor=\"".$my_colors["main_head_bg"]."\">\n";
				$base_url = "?m=webmail&folder=".urlencode($folder)."&start=$start&session=$user&sort_field=$sort_field&sort_order=$sort_order";
			...
			while (list ($key,$val) = each ($headers)) {
				...
				if ((($showdeleted==0)&&($deleted!="D")) || ($showdeleted)){
					if (($hideseen==0)||($seen=="N")){
						...
						$row["s"] = "<td><a href=\"?xa=read_message&m=webmail&".$args."\" ";
