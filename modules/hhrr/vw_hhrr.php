<?php /* COMPANIES $Id: vw_hhrr.php,v 1.2 2009-07-30 15:51:29 nnimis Exp $ */
##
...
$userTypes = arrayMerge(array("-1" => "User Type (All)"), $userTypes);
...
function showrowhhrr( &$a ) {
	...
	/*Si tiene permisos de RW sobre el modulo muestro el boton de borrar.
	 *Si ademas puede editar por lo menos una solapa muestro el de editar
	 */
	$canEditHHRR = CHhrr::canEdit($a["user_id"]);
	if ($canEditHHRR ){
		$canEditOneTab  = CHhrr::canEditOneTab($a["user_id"]);
		if ($canEditOneTab){
		...
		else{//Pongo un espacio para que quede alineado
			$s .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
		}
	...
	$s .= '</td>';
	$s .= '<td nowrap="nowrap">';
$s .= '<a href="./index.php?m=hhrr&a=viewhhrr&id='.$a["user_id"].'" title="'.$a["company_name"].'">'.$a["user_last_name"].", ".$a["user_first_name"].'</a>';
	$s .= '</td>';
	$s .= '<td>';
    ...
    if($hayskill==1)
	  $s .= "$resultsk <a href='./index.php?m=hhrr&a=viewhhrr&tab=7&id=".$a["user_id"]."'>[".$AppUI->_( 'View' )."]</a>" ;
	$s .= '</td>';
...
if ($AppUI->user_type != 1)
	$hhrr_permissions="hhrr_permissions.id_user=$user_id AND ( hhrr_permissions.company = -1 OR (hhrr_permissions.company=users.user_company AND hhrr_permissions.department=-1 ) OR (hhrr_permissions.company=users.user_company AND hhrr_permissions.department=users.user_department))";
else
	$hhrr_permissions=" 1 ";
	
$sql = "
SELECT users.*, company_name
FROM users, hhrr_permissions, companies
where $where
and companies.company_id=users.user_company
and $hhrr_permissions
...
$s = '<table width="100%" border="0" cellpadding="0" cellspacing="0" '.$tableStyle.' >';
$s .= '<tr>';
$s .= '<td>';
$s .= '&nbsp;';
$s .= '</td>';
...
if ($candidate != "1")
{
   ...
   $s .= "<a href=\"index.php?m=hhrr&tab=3&l=-1&utype=$utype\">".$AppUI->_('All')."</a>&nbsp;";
   foreach($letters as $letra){
   $s .= "<a href=\"index.php?m=hhrr&tab=3&l=$letra&utype=$utype\">$letra</a>&nbsp;";
   }
}
...
//Traigo las empresas que tengo permisos sobre el modulo de HHRR NO COMPANIES!!!
$companies=empresas_habilitadas_hhrr();
$companies=arrayMerge( array( '-1'=>$AppUI->_('Companies (All)')), $companies );

$departments=deptos_habilitados_hhrr($company_id);
$departments=arrayMerge( array( '-1'=>$AppUI->_('Departments (All)')), $departments );
...
$s .= '<td></td>';
$s .= '<td  align="center">';
$s .=  arraySelect( $companies, 'company_id', 'onChange="javascript: this.form.submit()" class="text"', $company_id,true, false,'145px');
$s .= '</td>';
$s .= '<td align="left">|</td>';
$s .= '<td  align="center">';
$s .=  arraySelect( $departments, 'dept_id', 'onChange="javascript: this.form.submit()" class="text"', $dept_id,true, false,'160px');
$s .= '</td>';
$s .= '<td align="left">|</td>';
$s .= '<td align="center">'.arraySelect( $userTypes, 'utype', 'class="text" size="1" onchange="javascript: this.form.submit()"', $utype, true,false,'145px');
$s .= '</tr></table></td>';
}else{
