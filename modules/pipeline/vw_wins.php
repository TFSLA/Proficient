<?php /* COMPANIES $Id: vw_wins.php,v 1.1 2009-05-19 21:15:44 pkerestezachi Exp $ */
##
...
if(!$delegator_id || ($delegator_id && $AppUI->user_id == $delegator_id))
{
	...
	{
		if (!count( $rows))
			$rows = $usr->getSalesPipelines($deleg["delegator_id"], "status in ('Win')" );
		else
			$rows = array_merge($usr->getSalesPipelines($deleg["delegator_id"], "status in ('Win')" ), $rows);
	}
}

if (!count( $rows))
	$rows = $usr->getSalesPipelines( 0, "status in ('Win')" );
else
	$rows = array_merge($usr->getSalesPipelines( 0, "status in ('Win')" ), $rows);
...
if ($canAdd)
{
	$s .= '<input type="button" class=button value="'.$AppUI->_( 'new lead' ).'" onClick="javascript:window.location=\'./index.php?m=pipeline&a=addedit&type=Win&delegator_id='.$delegator_id.'&dialog='.$dialog.'\';">';
}
$s .= '</td-->';
*/
$s .= '</tr>';
