...
//$titleBlock = new CTitleBlock( $AppUI->_('Private Messaging'), 'webmail.gif', $m, "$m.$a" );
$titleBlock = new CTitleBlock( $AppUI->_('Private Messaging'), 'webmail.gif', $m, "colaboration.index" );
//$titleBlock->addCrumb("?m=$m", $AppUI->_('Inbox'));
//$titleBlock->addCrumb("sentbox", $AppUI->_('Sentbox'));
$titleBlock->show();
...
<form method="post" name="privmsg_list" action="index.php?m=privmsg">
...
if($msgdays>0){
  ...
  if($msgdays==364)   $where = " AND date >= date_sub(now(), interval 254 day) ";
      ...
      if($deleteall!="") $result=mysql_query("DELETE FROM messages WHERE recipient_id = {$AppUI->user_id} AND folder='INBOX'");
      if($delete!=""){
        $result = mysql_query("SELECT message_id FROM  messages WHERE recipient_id = {$AppUI->user_id} AND folder='INBOX' AND saved<>'Y';");
      ...
      $result = mysql_query("SELECT user_username, user_first_name, user_last_name, message_id, subject, isread, sender_id, DATE_FORMAT(date,'%a %b %d, %Y %h:%i %p') as datefmt FROM  messages, users WHERE users.user_id = messages.sender_id AND recipient_id = {$AppUI->user_id} AND folder='INBOX' AND saved<>'Y' AND user_type <> 5 $where ORDER BY date DESC  LIMIT $initreg, $itemsperpage;");
      while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
?>
	<tr>
	  <?
...
?>
  ...
  <table class="privFooterCell" border="0" cellpadding="1" cellspacing="0" width="100%">
    ...
    <tr>
	  <td align="right">
		<input type="submit" name="save" value="<?php echo strtolower($AppUI->_('Save Marked')); ?>" class="buttonbig" />
...
<?
        $query="select count(*) FROM  messages, users WHERE users.user_id = messages.sender_id AND recipient_id = {$AppUI->user_id} AND folder='INBOX' AND saved<>'Y'";
