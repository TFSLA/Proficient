...
<form method="post" name="privmsg_list" action="index.php?m=privmsg&a=savebox">
  ...
  <table border="0" cellpadding="1" cellspacing="0" width="100%">
   ...
   <tr class="privHeaderCell">
        ...
        <td width="10%" align="center" class="boldblanco"><?php echo $AppUI->_('From'); ?></td>
...
if($msgdays>0){
  ...
  if($msgdays==364)   $where = " AND date >= date_sub(now(), interval 254 day) ";
      ...
      if($deleteall!="") $result=mysql_query("DELETE FROM messages WHERE recipient_id = {$AppUI->user_id} ");
      if($delete!=""){
        $result = mysql_query("SELECT message_id FROM  messages WHERE recipient_id = {$AppUI->user_id} AND saved='Y';");
      ...
      //$result = mysql_query("SELECT recipient_id, users.user_username, users.user_first_name, users.user_last_name, users2.user_username as recipient_username ,message_id, subject, isread, sender_id, DATE_FORMAT(date,'%a %b %c, %Y %h:%i %p') as datefmt FROM  messages LEFT JOIN users ON messages.sender_id=users.user_id  LEFT JOIN users users2 ON messages.recipient_id=users2.user_id WHERE (sender_id = {$AppUI->user_id} OR recipient_id = {$AppUI->user_id}) AND saved='Y' $where ORDER BY date DESC  LIMIT $initreg, $itemsperpage;");
      $result = mysql_query("SELECT recipient_id, users.user_username, users.user_first_name, users.user_last_name, message_id, subject, isread, sender_id, DATE_FORMAT(date,'%a %b %c, %Y %h:%i %p') as datefmt FROM  messages LEFT JOIN users ON messages.sender_id=users.user_id WHERE recipient_id = {$AppUI->user_id} AND saved='Y' $where ORDER BY date DESC  LIMIT $initreg, $itemsperpage;");
      while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
...
<?
        $query="select count(*) FROM  messages, users WHERE users.user_id = messages.sender_id AND recipient_id = {$AppUI->user_id} AND saved<>'Y'";
