...
<script type="text/javascript">
   ...
   <select name="canal_id" id="canal_id" class="text" style="font-size:10px; width: 160px " onchange="canal_project(document.editFrm.company_id.value, document.editFrm.canal_id.value, '','' )" >
	...
	<td nowrap="nowrap" >
		<select name="project_id" id="project_id"  onchange="canal_project(document.editFrm.company_id.value, document.editFrm.canal_id.value, document.editFrm.project_id.value,'' );" style="font-size:10px; width:160 px;">
        </select>
	</td>	
    ...
    <td nowrap="nowrap" >
		<select name="user_id" style="font-size:10px; width: 160 px;">
		</select>
    </td>
</tr>
...
if($do_report){
	...
	if ( count($task_list) == 0 ) {
		...
		for($i=0; $i<=$days_difference; $i++){
			...
			$day=utf8_encode(Date_Calc::getWeekdayAbbrname($actual_date->day, $actual_date->month, $actual_date->year, 3));
			$day = $AppUI->_($day);
			$pdfdata[count($pdfdata) - 1][] = $day; 
			$table_header .= "<th>$day</th>";
		...
		<?php		
			...
			IF ($total_hours_capacity>0) 	echo $AppUI->_("Percentage used").": ".number_format($allocated_hours_sum/$total_hours_capacity,2)*100 ."%<br />";
			ELSE echo $AppUI->_("Percentage used").": N/A <br />";
			...
			IF ($total_hours_capacity>0) $pdffooter[][] = $AppUI->_("Percentage used").": ".number_format($allocated_hours_sum/$total_hours_capacity,2)*100;			
			ELSE $pdffooter[][] = $AppUI->_("Percentage used").":  N/A"	?>
			</td>
			<td align="center">
		<?php
			$total_hours_capacity = $working_days_count*$AppUI->getConfig("daily_working_hours")*count($user_list);
			echo $AppUI->_("daily_working_hours")." :".$AppUI->getConfig("daily_working_hours")."<br>";
			echo "<h4>".$AppUI->_("Total capacity for all users")."</h4>";
			echo $AppUI->_("Allocated hours").": ".number_format($allocated_hours_sum,2)."<br />";
			echo $AppUI->_("Total capacity").": ".number_format($total_hours_capacity,2)."<br />";
			if ($total_hours_capacity >0 )
			{
			    echo $AppUI->_("Percentage used").": ".number_format($allocated_hours_sum/$total_hours_capacity,2)*100 ."%<br />";
			}else{
				echo $AppUI->_("Percentage used").":  N/A<br />";
			}
			...
			if ($total_hours_capacity > 0 )
			{
				...
				$pdffooter[][] = $AppUI->_("Percentage used").": N/A ";
			}
