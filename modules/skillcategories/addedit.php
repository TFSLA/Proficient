...
if (!db_loadHash( $sql, $drow ) && $id > 0) {
	$titleBlock = new CTitleBlock( 'Invalid Skill Category ID', 'hhrr.gif', $m, 'ID_HELP_HHRR_EDIT' );
...
function submitIt() {
	...
	if (form.name.value.length < 1) {
		alert( "Please enter a valid Skill Category name" );
		form.name.focus();
	} else {
		form.submit();
	}
...
<form name="editFrm" action="?m=skillcategories" method="post">
