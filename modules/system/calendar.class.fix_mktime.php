...
class CWorkCalendar extends CDate {
	...
	function addDays( $n ) {
		...
		if ($n > 0){
			...
			$this->setTime( $eval_date->getHour(), $eval_date->getMinute(), $eval_date->getSecond());
				...
				while (	$start_calendar <= $eval_date->format(FMT_TIMESTAMP_DATE) 	&&
					...
					if( $calday->calendar_day_working && !isset($this->_hollidays[$eval_date->format(FMT_TIMESTAMP_DATE)]) ){
						...
						while ($diff > 0 && $shift_duration > 0 && $j <= 5){
							...
							if ($inc > 0) $eval_date->addSeconds($inc);
							else $eval_date->subtractSeconds($inc);
							//$eval_date->addSeconds($inc * 60 * 60);
				...
				while (	$start_calendar <= $eval_date->format(FMT_TIMESTAMP_DATE) 	&&
					...
					if( $calday->calendar_day_working && !isset($this->_hollidays[$eval_date->format(FMT_TIMESTAMP_DATE)]) ){
						...
						while ($diff > 0 && $shift_duration > 0 && $j <= 5
							...
							//$eval_date->subtractSeconds($inc * 60 * 60);
							//$eval_date->setDate( $eval_date->getTime() - $inc * 60 * 60, DATE_FORMAT_UNIXTIME);														
	...
	function dateDiff(&$from_date, $duration_type=1){
		...
		for ($i=0;$i<count($cal_dates) && $diff > 0;$i++){
			...
			while (	$start_calendar <= $eval_date->format(FMT_TIMESTAMP_DATE) 	&&
				...
				if( $calday->calendar_day_working && !isset($this->_hollidays[$eval_date->format(FMT_TIMESTAMP_DATE)]) ){
					...
					if ($no_working){
						if ($n >=0){
							$from_date->setTime( $eval_date->getHour(), $eval_date->getMinute(), $eval_date->getSecond());
					...
					while ($diff > 0 && $shift_rest > 0 && $shift_duration> 0 && $j <= 5){
						...
						if ($shift_rest >= $shift_duration){
							...
							if ($shift_rest > 0) $eval_date->addSeconds($shift_rest);
							else $eval_date->subtractSeconds($shift_rest);
		...
		/* 03/06/2005 14.30  comentado para evitar el cambio de hora sin sentido bug 629
		if ($n >=0){
			$this->setDate($eval_date->getTime(), DATE_FORMAT_UNIXTIME);
		}else{
			$from_date->setDate($eval_date->getTime(), DATE_FORMAT_UNIXTIME);
	...
	function fitDateToCalendar(){
		...
		for ($i=0;$i<count($cal_dates) && $is_working==false;$i++){
			...
			while (	$start_calendar <= $eval_date->format(FMT_TIMESTAMP_DATE) 	&&
					$end_calendar > $eval_date->format(FMT_TIMESTAMP_DATE)		&& 
					$is_working==false){
				echo "&nbsp;&nbsp;841 - Eval Date: ".$eval_date->format(FMT_TIMESTAMP);		
				$cal = new CCalendar();		
				$cal = $this->_work_calendar[$start_calendar];
				$calday = new CCalendarDay();
				$day_id = $eval_date->getDayOfWeek() + 1;
				...
				if( $calday->calendar_day_working && !isset($this->_hollidays[$eval_date->format(FMT_TIMESTAMP_DATE)]) ){
					...
					for($j=1; $j<=5 && $is_working == false && $is_valid_shift; $j++){
						...
						if ($calday->$start_time !== NULL && $calday->$end_time !== NULL){
							...
							echo "&nbsp;&nbsp;&nbsp;&nbsp;861 - Shift start time: ".$start_time->format(FMT_TIMESTAMP_TIME)."<br>\n";
							
							echo "&nbsp;&nbsp;&nbsp;&nbsp;863 - Eval time: ".$eval_date->format(FMT_TIMESTAMP_TIME);
							...
							echo "<br>\n";
							echo "&nbsp;&nbsp;&nbsp;&nbsp;865 - Shift end time: ".$end_time->format(FMT_TIMESTAMP_TIME)."<br>\n";
						}else{
				...
				echo "&nbsp;&nbsp;&nbsp;&nbsp;890 - Eval time: ".$eval_date->format(FMT_TIMESTAMP_TIME);
				//establezco la hora a cero e incremento un dia en la fecha
				if ($is_working == false){
					$eval_date->setTime(0, 0, 0);
					$eval_date->setTime( $eval_date->getHour(), $eval_date->getMinute(), $eval_date->getSecond());
					//$eval_date->setDate( $eval_date->getTime() + 60 * 60 * 24 * 1, DATE_FORMAT_UNIXTIME);
				}
				echo "&nbsp;&nbsp;&nbsp;&nbsp;896 - Eval time: ".$eval_date->format(FMT_TIMESTAMP_TIME);
				echo "<br>\n";
			}
		}
		
		echo "&nbsp;&nbsp;&nbsp;&nbsp;901 - Eval time: ".$eval_date->format(FMT_TIMESTAMP_TIME);
		echo "&nbsp;&nbsp;&nbsp;&nbsp;902 - Eval gettime: ".$eval_date->getTime();
		echo "&nbsp;&nbsp;&nbsp;&nbsp;903 - this: ".$this->format(FMT_TIMESTAMP);
		$this->setTime( $eval_date->getHour(), $eval_date->getMinute(), $eval_date->getSecond());
		//$this->setDate( $eval_date->getTime(), DATE_FORMAT_UNIXTIME);
		echo "&nbsp;&nbsp;&nbsp;&nbsp;905 - this: ".$this->format(FMT_TIMESTAMP);
		echo " -->\n";
