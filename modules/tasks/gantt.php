<?php /* TASKS $Id: gantt.php,v 1.1 2009-05-19 21:15:46 pkerestezachi Exp $ */
/*
 * Gantt.php - by J. Christopher Pereira
 * TASKS $Id: gantt.php,v 1.1 2009-05-19 21:15:46 pkerestezachi Exp $
 */


include ($AppUI->getConfig('root_dir')."/lib/jpgraph2.0/src/jpgraph.php");
include ($AppUI->getConfig('root_dir')."/lib/jpgraph2.0/src/jpgraph_gantt.php");
...
//echo "<br>$project_id <pre>"; print_r($_GET); echo "</pre>";
...
$tsql = "SELECT $select FROM $from $join WHERE $where ORDER BY project_id, task_wbs_level,task_wbs_number, task_start_date";
...
//echo "<pre>"; print_r($projects[$row['task_project']]['tasks']);echo "</pre>";
//echo "El ancho es : ".$_GET[width]."<br>";
...
$tsql = "SELECT max(tasks.task_end_date) FROM $from $join WHERE $where";
$lastest_task = db_loadResult($tsql);
$project_end = ($projects[$project_id]["project_end_date"] > $lastest_task) ? $projects[$project_id]["project_end_date"] : $lastest_task;
...
if ($showWork=='1') {
	//$graph->scale->actinfo->SetColTitles(array( $AppUI->_('Task', UI_OUTPUT_RAW), $AppUI->_('Work', UI_OUTPUT_RAW), $AppUI->_('Start', UI_OUTPUT_RAW), $AppUI->_('Finish', UI_OUTPUT_RAW)),array(230,16, 60,60));
	$graph->scale->actinfo->SetColTitles(array( $AppUI->_('Task', UI_OUTPUT_RAW) ),null);
} else {
	//$graph->scale->actinfo->SetColTitles(array( $AppUI->_('Task', UI_OUTPUT_RAW), $AppUI->_('Dur.', UI_OUTPUT_RAW), $AppUI->_('Start', UI_OUTPUT_RAW), $AppUI->_('Finish', UI_OUTPUT_RAW)),array(230,16, 60,60));
	$graph->scale->actinfo->SetColTitles(array( $AppUI->_('Task', UI_OUTPUT_RAW) ),null);
}
...
if (is_file( TTF_DIR."arialbd.ttf" ))
	$graph->scale->tableTitle->SetFont(FF_ARIAL,FS_BOLD,11);
$graph->scale->SetTableTitleBackground("#".$projects[$project_id]["project_color_identifier"]);
$graph->scale->tableTitle->SetColor(bestColor( $projects[$project_id]["project_color_identifier"] ));
...
//echo "<pre>"; print_r($graph->img);echo "$tsql</pre>";
...
function showgtask( &$a, $level=0 ) {
	...
	global $gantt_arr, $AppUI;
   
	$gantt_arr[] = array($a, $level);
	$AppUI->gantt_arr = $gantt_arr;
}
...
$gantt_arr = $AppUI->gantt_arr;
//echo "<br>gantt_arr <pre>"; print_r($AppUI->gantt_arr); echo "</pre>";
...
$graph->Stroke($AppUI->getConfig('root_dir').'/files/temp/'.$tmp_gantt.'.png');
   
  
?>
<IMG SRC="files/temp/<?=$tmp_gantt?>.png" BORDER="0" >
