<?php /* TASKS $Id: organizer.php,v 1.1 2009-05-19 21:15:46 pkerestezachi Exp $ */
...
function fixate_task($task_index, $time, $dep_on_task) {
	...
	foreach($tasks as $task2) {
		...
		if($task2["fixed"] && (
			...
			if(!$option_advance_if_possible || $task2["task_manual_percent_complete"] != 100) {
	...
	if(mysql_num_rows($query) != 0) {
		...
		while($d < count($dependencies)) {
			...
			if(!$tasks[$index]["fixed"]) {
				...
				if(!$option_advance_if_possible || $tasks[$index]["task_manual_percent_complete"] != 100) {
					// get latest end_date
...
<tr>
	<td><?php echo dPshowImage( dPfindImage( 'tasks.gif', $m ), 20, 20, '' ); ?></td>
...
if($do != "conf") {
	...
	while ($row = mysql_fetch_array( $dtrc, MYSQL_ASSOC )) {
		...
		// check delayed tasks
		if($do == "ask") {
			if(!$row["task_dynamic"] && $row["task_manual_percent_complete"] == 0) {
