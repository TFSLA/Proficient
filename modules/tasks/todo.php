<?php /* TASKS $Id: todo.php,v 1.1 2009-05-19 21:15:46 pkerestezachi Exp $ */
...
if ($selected && count( $selected )) {
	foreach ($selected as $key => $val) {
		if ( $task_priority == 'c' ) {
			...
			$sql = "UPDATE tasks SET task_complete=1 WHERE task_id=$val";
			//FIXME: habria que usar la funcion de aca abajo para operar con tareas
			//$ueue = new CTask($val);
			//$ueue->task_complete_set("1");
...
$sql = "
		 ...
		 FROM projects 
                         INNER JOIN tasks as a ON a.task_project = project_id
                         LEFT JOIN user_tasks ON user_tasks.task_id = a.task_id
...
foreach ($tasks as $a) {
	...
	if ($now->after( $start ) && $a["task_complete"] == 0) {
...
<tr>
	<td>
<?php if ($canEdit) { ?>
		<a href="./index.php?m=tasks&a=addedit&task_id=<?php echo $a["task_id"];?>"><img src="./images/icons/edit_small.gif" alt="<?=$AppUI->_('Edit Task')?>" border="0" width="20" height="20"></a>
<?php } ?>
	</td>
	<td align="right">
