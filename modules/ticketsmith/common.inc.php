...
/* $Id: common.inc.php,v 1.1 2009-05-19 21:15:46 pkerestezachi Exp $ */

/* program info */
$program = "PSA";
$version = "0.6.3";
$xmailer = "psa (http://psa.tfsla.com/)";
...
function get_time_ago ($timestamp) {
    ...
    $output = "$interval ".$AppUI->_($output)." ".$AppUI->_("ago");
...
function format_field ($value, $type, $ticket = NULL) {
    ...
    switch ($type) {
        ...
        case "assignment":
            $options[0] = "-";
            $query = "SELECT user_id as id, CONCAT_WS(' ',user_first_name,user_last_name) as name FROM users where user_type <> 5 order by user_first_name,user_last_name";
            $result = do_query($query);
        ...
        case "view":
            ...
            $output .= "<img src=images/icons/edit_small.gif border=0></a>";
            break;
	case "attach":
	...
	case "doattach":
        ...
        case "activity_date":
            ...
            break;
        case "elapsed_date":
						//Agregado para localizar el formato de la fecha
						$df = $AppUI->getPref('SHDATEFORMAT')." ".$AppUI->getPref('TIMEFORMAT');
						$date = new CDate( date('Y-m-d H:i:s' , $value) );
            $output =  $date->format( $df ); 
						// Fin localizacion
