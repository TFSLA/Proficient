<?php /* TASKS $Id: addtime_new.php,v 1.2 2009-06-26 17:43:24 pkerestezachi Exp $ */
...
//require_once( "./classes/companies.class.php" );
...
$CBugs->loadProjects();
$CBugs->addItemAtBeginOfProjects($CBugs->addItem(0, "Not Specified"));
...
function addhour(){
	...
	var old_project_task = new Array();
	var old_project_bug = new Array();
	...
	var old_appid = new Array();
	      ...
	      for(var i = 0; i < dates.length; i++){
            ...
            old_project_task[old_project_task.length] = project_task[i];
		    old_project_bug[old_project_bug.length] = project_bug[i];
			...
			var new_project_task = new Array();
			var new_project_bug = new Array();
		   ...
		   // Calculo los dias , la cantidad de horas por dia //
			...
			for (var i = 0; i<dif; i++){
                ...
                new_project_task[i] = parseInt(frm.project_task.value);
				new_project_bug[i] = parseInt(frm.project_bug.value);
			...
			var temp_project_task = new Array();
			var temp_project_bug = new Array();
					...
					for(var j = 0; j < old_dates.length; j++)
					 ...
					 if (gd)
				     {
					   cant = cant + new_hours[i];
						  ...
						  temp_project_task[k] = new_project_task[i];
						  temp_project_bug[k] = new_project_bug[i];
			...
			var final_project_task = new Array();
			var final_project_bug = new Array();
			...
			for(var i = 0; i < total; i++){
				...
				if(i < old_dates.length)
				{
				 ...
				 final_project_task[i] = old_project_task[i];
				 final_project_bug[i] = old_project_bug[i];
				...
				final_project_task[i] = temp_project_task[ind];
				final_project_bug[i] = temp_project_bug[ind];
	...
	project_task = final_project_task;
	project_bug = final_project_bug;
...
function genera_vector(){
		...
		// Calculo los dias , la cantidad de horas por dia //
			...
			project_task = new Array();
			project_bug = new Array();
			task_id = new Array();
			bug_id = new Array();
			descrip = new Array();
			billable = new Array();
			...
			for (var i = dates.length; i<dif; i++){
                ...
                project_task[i] = parseInt(frm.project_task.value);
				project_bug[i] = frm.project_bug.value;
...
function build_table_times(){
	...
	var html_header ='<table cellspacing="1" cellpadding="2" border="0" width="99%" style="background-color:#ffffff;"><tr style="background-color: #000000; font-weight: bold; color: #FFFFFF;"><th><?php echo $AppUI->_("Times");?>:</th><th align="right"><?php echo $AppUI->_("Start Time");?></th><th align="left"><?php echo $AppUI->_("End Time");?></th><th align="left"><?php echo $AppUI->_($label_value);?></th><th align="right" ><?php echo $AppUI->_("Applied to");?></th><th align="right" ><?php echo $AppUI->_("Project");?></th><th align="right" ><?php echo $AppUI->_("Name");?></th><th align="right" ><?php echo $AppUI->_("Description");?></th><th align="right" ><?php echo $AppUI->_("Billable");?></th><th align="right" ><?php echo $AppUI->_("Company");?></th><th align="right"></th><th align="right"></th></tr>';
	var html_footer = '</table>';
        
	var no_data	='<tr style="background-color:#ffffff;"><td colspan="12"><?php echo $AppUI->_("No data available");?></td></tr>';
	var row		='<tr><td class="hilite" width="80">[DATE]</td><td class="hilite" align="right" >[START_HOURS]</td><td class="hilite" align="right">[END_HOURS]</td><td class="hilite" align="right" >[HOURS]</td><td class="hilite" align="right" width="60px">[APPLIED]</td><td class="hilite" align="right" width="60px">[PROJECT]</td><td class="hilite" align="right" width="60px">[NAME]</td><td class="hilite" align="right" width="60px">[DESCRIP]</td><td class="hilite" align="right" width="60px">[BILABLE]</td><td class="hilite" align="right" width="60px">[COMPANY]</td><td class="hilite" width="20px">[DEL]</td><td class="hilite" width="20px">[DUPLICA]</td></tr>';
	...
	if (dates.length == hours.length && dates.length > 0){
		...
		for(var i = 0; i < dates.length; i++){
			...
			var company_field ='<? echo $CProjects->generateHTMLcboCompanies("''","text"); ?>';
			...
			var applied_to_field = '<select name="applied_to_type[]" size="1" class="text" onchange="javascript:changeApplied_table(' + i +',this);" ><option value="1" '+ sel1 +'>Task</option><option value="2" '+ sel2 +'>Bug</option><option value="3" '+ sel3 +'>Nothing</option></select>';
			var descrip_field = '<input type="text" class="text" name="timexp_description[]" value="'+ descrip[i]+'" maxlength="255" size="25" onblur="updatedescript(' + i +',this);" />';

            
			if (applied_to_type[i]==1)
            {
			/* Projectos-Tareas */
			var project_t_field = '<? echo $CProjects->generateHTMLcboProjects_tabla("'+ project_task[i] +'","text"," onchange=\"javascript:changeTask_table(' + i +',this);\" "); ?>';
            var select = 'value="'+ project_task[i] +'"';
            var select2 = 'value="'+ project_task[i] +'" selected';
		    
            project_t_field = project_t_field.replace(select,select2);
			...
			var select1 = '<select name=\"named[]\" class=\"text\" onchange=\"javascript: changeTaskSel_table(' + i +',this);\" >';
			var select3 = '</select>';
            
			for(var k = 0; k < Temp.length; k++){

			  if (Temp[k][0]==project_task[i])
				{   
				    if (named[i]==Temp[k][1])
					{
					var sel = 'selected';
                    ...
                    select2 += '<option value="'+Temp[k][1]+'" '+ sel +'>'+Temp[k][2]+'</option>';
				}
			}
			...
			var bilable_field = '<select name="timexp_billable_box[]" size="1" class="text" onchange=\"javascript: changebilableSel_table(' + i +',this);\"><option value="0" '+ sel1 +'>No</option><option value="1" '+ sel2 +'>Yes</option></select>';
            ...
            {
			/* Projectos - Bugs */
			var project_t_field = '<? echo $CBugs->generateHTMLcboProjects_tabla("'+ project_bug[i] +'","text"," onchange=\"javascript:changeBug_table(' + i +',this);\" "); ?>';

			var select = 'value="'+ project_bug[i] +'"';
            var select2 = 'value="'+ project_bug[i] +'" selected';
			...
			var bilable_field = '<select name="timexp_billable_box[]" size="1" class="text" onchange=\"javascript: changebilableSel_table(' + i +',this);\"><option value="0" '+ selb1 +'>No</option><option value="1" '+ selb2 +'>Yes</option></select>';
			...
			if (named[i]=="Reunión con Clientes")
				{
				var selt2 = 'selected';
				}
				else
			...
			if (named[i]=="Generación de propuesta")
				{
				var selt4 = 'selected';
				}
				else
			...
			if (named[i]=="Generación de doc. interna")
				{
				var selt7 = 'selected';
				}
				else
			...
			var name_field = '<select class="text" name="named[]" size="1" onchange=\"javascript: changeNothingSel_table(' + i +',this);\"><option value="Reuni&oacute;n Interna" '+selt1+'>Reuni&oacute;n Interna</option><option value="Reuni&oacute;n con Clientes" '+selt2+'>Reuni&oacute;n con Clientes</option><option value="Viaje" '+selt3+'>Viaje</option><option value="Generaci&oacute;n de propuesta" '+selt4+'>Generaci&oacute;n de propuesta</option><option value="Soporte equipos" '+selt5+'>Soporte equipos </option><option value="Entrenamiento" '+selt6+'>Entrenamiento</option><option value="Generaci&oacute;n de doc. interna" '+selt7+'>Generaci&oacute;n de doc. interna</option><option value="Otros" '+selt8+'>Otros</option></select>';
			...
			content += row.replace('[DATE]', date_field).replace('[START_HOURS]', start_hour_field).replace('[END_HOURS]',ends_hour_field).replace('[HOURS]', hour_field).replace('[APPLIED]', applied_to_field).replace('[PROJECT]', project_field).replace('[NAME]', name_field).replace('[DESCRIP]', descrip_field).replace('[BILABLE]', bilable_field).replace('[COMPANY]', company_field).replace('[DEL]', del_field).replace('[DUPLICA]', dup_field); 
		}
...
function changeTask_table(ind,obj){
    var frm = document.forms["batchFrm"];
	...
	var old_appid = new Array();
	      ...
	      for(var i = 0; i < dates.length; i++){
			...
			if (ind==i){
			  ...
			  var new_project_task = project_task[i];
			  var new_project_bug = project_bug[i];
		   ...
		   if ((canth > 24 )&&(rta==true))
			...
			for(var i = 0; i < total; i++){
				...
				if(i < old_dates.length)
				{
				 ...
				 final_project_task[i] = new_project_task;
				 final_project_bug[i] = new_project_bug;
...
function delhour(ind){
	...
	var tmp_project_task = new Array();
	var tmp_project_bug = new Array();
	...
	for(var i = 0; i < dates.length; i++){
		if(ind!=i){
            ...
            tmp_project_task[tmp_project_task.length] = project_task[i];
		    tmp_project_bug[tmp_project_bug.length] = project_bug[i];
    ...
    project_task = tmp_project_task;
	project_bug = tmp_project_bug;
	task_id = tmp_task_id;
    bug_id = tmp_bug_id;
	descrip = tmp_descrip;
...
function validateTimexp(){
	...
	// Reviso el vector para asegurarme que todos los registros tengan cargado el nombre //
	 ...
	 if(completos ==1){
		alert("<?php echo $AppUI->_("timexpInvalidName");?>");
		rta = false;
	}
...
function popProject() {	window.open('./index.php?m=public&a=selector_timex&dialog=1&suppressLogo=1&callback=setProject&table=projec', 'selector', 'left=50,top=50,height=300,width=400,resizable')
}


function setProject( key, val ) {
	var f = document.batchFrm;
...
<?php 
		...
		$CProjects->setCboProjects("project_id");
		$CProjects->setCboTasks("task_id");
		$CProjects->setJSSelectedTask("task_id");*/

		$CProjects->setFrmName("batchFrm");
		$CProjects->setCboCompanies("company_id");
		$CProjects->setCboProjects("project_id");
		$CProjects->setJSSelectedProject("project_id");
		
		echo $CProjects->generateJS();

		$CBugs->setFrmName("batchFrm");
		$CBugs->setCboProjects("project_id");
...
<tr>
	...
	<?php /* 
			...
			*/ ?>
			...
			<col width="40px"><col  width="150px"><col>
			...
			<tr> 
			  ...
			  <td align="left">
				&nbsp;<?php echo $CProjects->generateHTMLcboCompanies($company, "text");?>
			  </td>
			</tr>
			</table>
			<br>
			...
			<tr> 
			  ...
			  <td align="left">
				...
				&nbsp;<?php echo $CProjects->generateHTMLcboProjects($project, "text");?>
			  </td>
			<?php } ?>
			</tr>
			...
			<!-- <table width="200" border="0" cellspacing="0" cellpadding="0">
			<col width="42px"><col  width="150px"><col>
		...
		*/ ?>
		...
		<table width="200" border="0" cellspacing="0" cellpadding="0">
		<col width="50px"><col  width="165px"><col>
		<?php if (!$external){ ?>
		...
		<tr><td ><!-- <?php echo $AppUI->_("Task").":";?> --></td>
			<td>
			<!-- <?php //echo $CProjects->generateHTMLcboTasks($task, "text"); ?> -->
			</td>
		...
		*/ ?>
		...
		<tr><td ><?php echo $AppUI->_('Name');?>: </td>
			<td>
			<select class='text' name='timexp_name2'>
				...
				<option value='Generaci&oacute;n de doc. interna'>Generaci&oacute;n de doc. interna</option>
		...
		*/ ?>
		<div id="incidencediv" style="display: none;">
			   <br>
			   <table width="200" border="0" cellspacing="0" cellpadding="0">
				<col width="48px"><col  width="165px"><col>
...
<tr>
  <td colspan="4" align="right">
       ...
       <input type="button" class="button" name="generate" value="<?php echo $AppUI->_('generate');?>" onclick="generate_times('1');" style="display: true;"/>
