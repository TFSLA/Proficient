...
for($i=0; $i<count($tslist);$i++){
	...
	$next_status = $timexp_status;//explode(",",$ts_status_transition[$row["timesheet_last_status"]]);
...
?>
    <tr <?php echo $bgcolor;?>>
      ...
      $ns_keys = array_keys($next_status);
      for($i=0; $i<count($ns_keys); $i++){
      	if ( ! ($tsobj->canChangeStatus($ns_keys[$i]))){
      		unset ($next_status[$ns_keys[$i]]);
      	}
      }
      $next_status = arrayMerge(array("-1"=>"-------"), $next_status);
			
      echo arraySelect($next_status, 'timesheetstatus_status['.$row["timesheet_id"].']','size="1" class="text"', "", true ) ;
