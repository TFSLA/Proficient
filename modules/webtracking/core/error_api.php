<?php
	...
	function error_handler( $p_type, $p_error, $p_file, $p_line, $p_context ) {
		...
		switch ( $p_type ) {
			case E_WARNING:
				$t_error_type = $AppUI->_('SYSTEM WARNING');
				...
				break;
			case E_NOTICE:
				$t_error_type = $AppUI->_('SYSTEM NOTICE');
				$t_error_description = $p_error;
				...
				break;
			case E_USER_ERROR:
				$t_error_type = $AppUI->_('APPLICATION ERROR')." #$p_error";
				$t_error_description = error_string( $p_error );
				$t_method = 'halt';
				break;
			case E_USER_WARNING:
				$t_error_type = $AppUI->_('APPLICATION WARNING')." #$p_error";
				$t_error_description = error_string( $p_error );
				if ( ON == config_get( 'show_warnings' ) ) {
					$t_method = 'inline';
				}
		...
		if ( 'halt' == $t_method ) {
			...
			$AppUI->setMsg($t_error_description, UI_MSG_ERROR);
			$redir = substr($_SERVER["HTTP_REFERER"], strpos($_SERVER["HTTP_REFERER"],"?")+1);
