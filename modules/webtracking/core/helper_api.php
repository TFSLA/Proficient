<?php
	...
	# --------------------
	# get the class string for the given status
	function get_status_class( $p_status ) {
		$t_status_enum_string = config_get( 'status_enum_string' );
		$t_status_colors = config_get( 'status_colors' );
	...
	function helper_get_current_task() {
		$t_cookie_name = config_get( 'task_cookie' );

		$t_task_id = gpc_get_cookie( $t_cookie_name, null );

		if ( $t_task_id == "" ) {
			$t_task_id = 0;
		}

		return (int)$t_task_id;
	}
	...
	function helper_set_current_task( $p_task_id ){
		$t_task_cookie_name	= config_get( 'task_cookie' );
		$t_filter_cookie_name	= config_get( 'view_all_cookie' );

		gpc_set_cookie( $t_task_cookie_name, $p_task_id, true );
...
*/
		...
		foreach ( gpc_strip_slashes( $_POST ) as $key => $val ) {
			$key = htmlspecialchars( $key );
			$val = htmlspecialchars( $val );
			echo $key . "=" . $val . "&";
		}
		foreach ( gpc_strip_slashes( $_GET ) as $key => $val ) {
			$key = htmlspecialchars( $key );
			$val = htmlspecialchars( $val );
			echo $key . "=" . $val . "&";
		}
		...
		echo '<br /><br /><input type="button" class="button" onClick="history.back()" value="' .lang_get( 'cancel_bug_button' ). '" /><input type="submit" class="button" value="' . $p_button_label . '" />';
		echo "\n</form>\n";
