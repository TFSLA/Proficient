<?php
	...
	function history_get_events( $p_bug_id ) {
		...
		$query = "SELECT b.*, u.user_username as username
	...
	function history_get_events_array( $p_bug_id ) {
		...
		$query = "SELECT b.*, UNIX_TIMESTAMP(b.date_modified) as date_modified, u.user_username as username
		...
		for ( $i=0; $i < $history_count; $i++ ) {
			...
			$v_date_modified = format_date( config_get( 'normal_date_format' ), $v_date_modified );
			...
			case 'eta':
				...
				break;
			case 'date_deadline':
				if (strlen($v_old_value)>0 && $v_old_value>0)
					$v_old_value = format_date( config_get( 'short_date_format' ), $v_old_value );
				else
					$v_old_value =  "";

				if (strlen($v_new_value)>0 && $v_new_value>0)
					$v_new_value = format_date( config_get( 'short_date_format' ), $v_new_value );
				else
					$v_new_value =  "";
				$t_field_localized = lang_get( 'date_deadline' );
				break;
			...
			case 'date_from':
				...
				if (strlen($v_new_value)>0 && $v_new_value>0)
					...
					$v_new_value =  "";
				$t_field_localized = lang_get( 'date_from' );
				break;
			...
			case 'date_to':
				...
				if (strlen($v_new_value)>0 && $v_new_value>0)
					...
					$v_new_value =  "";
				$t_field_localized = lang_get( 'date_to' );
				break;
			...
			if ( NORMAL_TYPE != $v_type ) {
				...
				case BUG_ADD_RELATIONSHIP:
					$t_note = lang_get( 'relationship_added' ) . ': ' . lang_get( 'bug' ) . ' ' . bug_format_id( $v_new_value );
					break;
				case BUG_DEL_RELATIONSHIP:
					$t_note = lang_get( 'relationship_deleted' ) . ': ' . lang_get( 'bug' ) . ' ' . bug_format_id( $v_new_value );
					break;
				case BUG_CLONED_TO:
					$t_note = lang_get( 'bug_cloned_to' ) . ': ' . bug_format_id( $v_new_value);
					break;
				case BUG_CREATED_FROM:
					$t_note = lang_get( 'bug_created_from' ) . ': ' . bug_format_id( $v_new_value );
					break;
				# MASC RELATIONSHIP
				}
			}
	...
	function history_get_raw_events_array( $p_bug_id ) {
		...
		# grab history and display by date_modified then field_name
		$query = "SELECT b.*, u.user_username
				FROM $t_mantis_bug_history_table b
				LEFT JOIN $t_mantis_user_table u
				ON b.user_id=u.user_id
				WHERE bug_id='$c_bug_id'
		...
		for ( $i=0; $i < $raw_history_count; ++$i ) {
			...
			$raw_history[$i]['date']	= format_date( config_get( 'short_date_format' ), $v_date_modified );
