<?php
	...
	function summary_print_by_enum( $p_enum_string, $p_enum ) {
		...
		if ( ALL_PROJECTS == $t_project_id ){ 
			...
			$t_project_perm = current_user_get_accessible_projects(); 
		            $t_project_filter = " project_id IN ('".implode("','", $t_project_perm)."')";
		}else{ 
			...
			$label = $last_category;
				if ( ( ON == $t_summary_category_include_project ) 
				  && ( ALL_PROJECTS == $t_project_id ) ) {
					$query = "SELECT project_name as name
							FROM $t_mantis_project_table
							WHERE project_id=$last_project";
					$result2 = db_query( $query );
					$row2 = db_fetch_array( $result2 );
	...
	function summary_print_by_project() {
		...
		$query = "SELECT project_id, status FROM $t_mantis_bug_table
		                 WHERE project_id IN ('".implode("','", $t_project_perm)."')
			    ORDER BY project_id";
		$result = db_query( $query );
		...
		while ( $row = db_fetch_array( $result ) ) {
			extract( $row, EXTR_PREFIX_ALL, 'v' );
//echo "vp:".$v_project_id."<br>";
			if ( $v_project_id != $t_last_project 
			  && $t_last_project != -1 ) {
				$query = "SELECT project_name as name
						FROM $t_mantis_project_table
						WHERE project_id=$t_last_project";
				$result2 = db_query( $query );
				$row2 = db_fetch_array( $result2 );
				summary_helper_print_row( 
				  $row2['name']
		...
		if ( 0 < $t_bugs_total ) {
			$query = "SELECT project_name as name FROM $t_mantis_project_table
					      WHERE project_id=$t_last_project";
			$result2 = db_query( $query );
			$row2 = db_fetch_array( $result2 );
			summary_helper_print_row(
