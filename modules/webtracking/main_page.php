...
<?php
	...
	//PSA_MOD:
	require_once( 'view_all_bug_page.php' );
...
<?php
	if ( !current_user_is_anonymous() ) {
		...
		echo '<a href="index.php?m=webtracking&a=view_all_set&type=1&amp;reporter_id=any&amp;show_status=any&amp;show_severity=any&amp;show_category=any&amp;handler_id=' .  auth_get_current_user_id() . '&amp;hide_closed=on&amp;hide_resolved=on">' . current_user_get_assigned_open_bug_count() . '</a>';
		echo '</div>';

		echo '<div class="quick-summary-right">';
		echo lang_get( 'open_and_reported_to_me' ) . ':';
		echo '<a href="index.php?m=webtracking&a=view_all_set&type=1&amp;reporter_id=' . auth_get_current_user_id() . '&amp;show_status=any&amp;show_severity=any&amp;show_category=any&amp;handler_id=any&amp;hide_closed=on&amp;hide_resolved=on">' . current_user_get_reported_open_bug_count() . '</a>';
		echo '</div>';
...
<div align="center">
<?php
	print_bracket_link( 'index.php?m=webtracking&a=news_list_page', lang_get( 'archives' ) );
	$f_offset_next = $f_offset + $t_news_view_limit;
	$f_offset_prev = $f_offset - $t_news_view_limit;

	if ( $f_offset_prev >= 0) {
		print_bracket_link( 'index.php?m=webtracking&a=main_page&offset=' . $f_offset_prev, lang_get( 'newer_news_link' ) );
	}
	if ( $news_count == $t_news_view_limit ) {
		print_bracket_link( 'index.php?m=webtracking&a=main_page&offset=' . $f_offset_next, lang_get( 'older_news_link' ) );
	}
?>
</div>
