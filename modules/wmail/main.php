...
$clock->register("includes done");
	...
	if ( $folder=="INBOX"){
		if ($my_prefs["refresh_folderlist"]){
			...
			echo '<META HTTP-EQUIV="refresh"  CONTENT="'.$interval.';URL='.$_SERVER["REQUEST_URI"].'">';
			echo "\n";
		} 	
    ...
    //incluidos en optionfolders.inc.php
	...
	*/
	...
	echo "<!--td align=left valign=bottom>\n";
		...
		if (strcasecmp("INBOX", $folder)==0)
			echo "[<a href=\"?m=wmail&tab=0&a=bridge&session=$user&folder=$folder\" class=\"mainHeadingSmall\">".$mainStrings[17]."</a>]";
        if (strcmp($folder,$my_prefs["trash_name"])!=0)
			echo "[<a href=\"?m=wmail&tab=0&a=bridge&session=$user&folder=$folder&delete_all=1\" class=\"mainHeadingSmall\">".$mainStrings[18]."</a>]";
		echo "</span>\n";
	echo "</td-->\n";
	echo "<td align=\"right\" valign=\"bottom\" class=\"mainHeadingSmall\">";
		...
		if ($my_prefs["show_quota"]=="m"){
			...
			include("./modules/wmail/lang/".$my_prefs["lang"]."quota.inc");
	...
	if ($delete_all==1){
		...
		echo "<span class=\"small\">[<a href=\"?m=wmail&tab=0&a=bridge&session=$user&folder=$folder&delete_all=2&delete_all_num=$total_num&submit=Delete\">";
			echo $mainStrings[18]."</a>]</span>";
		echo "<span class=\"small\">[<a href=\"?m=wmail&tab=0&a=bridge&session=$user&folder=$folder\">".$mainStrings[19]."</a>]</span>";
	}
		...
		if (sizeof($headers)>0){			
			...
			if ($num_items > $num_show){
				if ($prev_start < $start){
					...
					echo "[<a href=\"?m=wmail&tab=0&a=bridge&session=$sid&folder=".urlencode($folder).$args."\" class=\"mainLightSmall\">";
			...
			***/
            ...
            //echo "<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"1\" bgcolor=\"".$my_colors["main_hilite"]."\">\n";
            ...
            //echo "<tr bgcolor=\"".$my_colors["main_head_bg"]."\">\n";
				...
				$check_link.="document.write(\"<a href=javascript:SelectAllMessages(true) class=''><b>+</b></a><span class=tblheader>|</span><a href=javascript:SelectAllMessages(false)><b>-</b></a>\")";
				$check_link.="\n--></SCRIPT><NOSCRIPT>";
				$check_link.="<a href=\"?m=wmail&tab=0&a=bridge&folder=".urlencode($folder)."&start=$start&session=$user&sort_field=$sort_field&sort_order=$sort_order&check_all=1\"><b>+</b></a>|";
 				$check_link.="<a href=\"?m=wmail&tab=0&a=bridge&folder=".urlencode($folder)."&start=$start&session=$user&sort_field=$sort_field&sort_order=$sort_order&uncheck_all=1\"><b>-</b></a>";
				$check_link.="</NOSCRIPT>";
				$tbl_header["c"] = "\n<td class=\"tableHeaderText\">$check_link</td>";
				$tbl_header["s"] = "\n<td class=\"tableHeaderText\">".FormFieldHeader("subject", $mainStrings[6])."</td>";
				if ($showto)
					$tbl_header["f"] = "\n<td class=\"tableHeaderText\">".FormFieldHeader("to", $fromheading)."</td>";
				else
					$tbl_header["f"] = "\n<td class=\"tableHeaderText\">".FormFieldHeader("from", $fromheading)."</td>";
				$tbl_header["d"] = "\n<td class=\"tableHeaderText\">".FormFieldHeader("date", $mainStrings[9])."</td>";
				if ($my_prefs["show_size"])
					$tbl_header["z"] = "\n<td class=\"tableHeaderText\">".FormFieldHeader("sisssze", $mainStrings[14])."</td>";
				$tbl_header["a"] = "<td><img src=\"modules/wmail/themes/".$my_prefs["theme"]."/images/att.gif\"></td>";
				$tbl_header["m"] = "<td><img src=\"modules/wmail/themes/".$my_prefs["theme"]."/images/reply.gif\"></td>";
				for ($i=0;$i<$num_cols;$i++) echo $tbl_header[$my_prefs["main_cols"][$i]];
			echo "\n</tr>\n";
			if ($MOVE_FIELDS){
				echo "<tr bgcolor=\"".$my_colors["main_head_bg"]."\">\n";
				$base_url = "?m=wmail&tab=0&a=bridge&folder=".urlencode($folder)."&start=$start&session=$user&sort_field=$sort_field&sort_order=$sort_order";
			...
			while (list ($key,$val) = each ($headers)) {
				...
				if ((($showdeleted==0)&&($deleted!="D")) || ($showdeleted)){
					if (($hideseen==0)||($seen=="N")){
						...
						echo "\n<tr id='msg_$id' class='".($seen=="Y"?"msg_read": "msg_unread")."'>\n";
						$js_over = " onmouseover=\"javascript:".
								"document.getElementById('msg_$id').style.backgroundColor='#E8E8E8';".
								"document.getElementById('msg_$id').style.color='#FF9900';\""
								." onmouseout=\"javascript:".
								"document.getElementById('msg_$id').style.backgroundColor='#FFFFFF';".
								"document.getElementById('msg_$id').style.color='#000000';\"";
						$js_over = "";
                        //show checkbox
						$row["c"] = "<td $js_over><input type=\"checkbox\" name=\"checkboxes[]\" value=\"$id\" ";
						...
						$msg_url = "?xa=read_message&m=wmail&tab=0&a=bridge&".$args;
						$js_event = " onmouseup=\"document.location='{$msg_url}';\"";
						$js_event = "";
						
						$row["s"] = "<td $js_event $js_over><a href=\"".$msg_url."\" ";
						$row["s"].= ($my_prefs["view_inside"]!=1?"target=\"scr".$user.urlencode($folder).$id."\"":"").">";
						$row["s"].= encodeUTFSafeHTML(LangDecodeSubject($subject, $my_prefs["charset"]))."</a></td>\n";
						//echo $row["s"];
						
						//show sender||recipient
						
						$row["f"] = "<td $js_event $js_over><a href=\"".$msg_url."\" ";
						$row["f"].= ($my_prefs["view_inside"]!=1?"target=\"scr".$user.urlencode($folder).$id."\"":"").">";
						if ($showto) $row["f"] .= encodeUTFSafeHTML(LangDecodeSubject($header->to, $my_prefs["charset"]))."</a></td>\n";						
						else $row["f"] .= encodeUTFSafeHTML(LangDecodeSubject($header->from, $my_prefs["charset"]))."</a></td>\n";
						...
						*///echo $row["f"];
						...
						$row["d"] = "<td $js_event $js_over><nobr>".ShowShortDate($timestamp, $lang_datetime)."&nbsp;</nobr></td>\n";
						//echo $row["d"];

						//show size
						if ($my_prefs["show_size"]==1){
							$row["z"] = "<td $js_event $js_over><nobr>".ShowBytes($header->size)."</nobr></td>\n";
							//echo $row["z"];
						}

						//attachments?
						$row["a"] = "<td $js_event $js_over>";

						if (preg_match("/multipart/i", $header->ctype)==TRUE){
							$row["a"].= "<img src=\"images/icons/stock_attach-16.png\">";
						}
						$row["a"].= "</td>\n";
						//echo $row["a"];

						//show flags
						$row["m"] = "<td $js_event $js_over>&nbsp;".
								($header->deleted?"D":"").
								($header->answered?
									"<img src=\"themes/".$my_prefs["theme"]."/images/reply.gif\">"
									:"").
								($header->priority < 3?
									dPshowImage(dPfindImage('mail_priority_high.gif'),'12','12','High priority') :
									"").
            ...
            if ($num_items > $num_show){
                ...
                echo "<select name=start class=\"small\" onchange=\"this.form.submit();\">\n";
                    $c=0;
                    while ($c < $total_num){
                        $c2=($c + $num_show);
                        if ($c2 > $total_num) $c2=$total_num;
                ...
                echo "</select>";
                //echo "<input type=\"submit\" class=\"button\" value=\"".$mainStrings[16]."\">";
